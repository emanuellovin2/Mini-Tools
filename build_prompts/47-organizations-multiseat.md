...
- **Backfill migration (pre-launch clean-break):** for every existing profile, create a personal org, make them owner, and **repoint all existing ownership** (`apps.vendor_id`, `reseller_offers.reseller_id`, `affiliate_links.affiliate_id`, `reseller_subscriptions.reseller_id`, `profiles.stripe_account_id` → org) to the personal org. Add `org_id` columns, backfill, then make NOT NULL.
...
### 4. RLS rewrite (org-membership based)
