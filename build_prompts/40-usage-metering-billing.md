...
### 5. Settlement job (durable, NOT fire-and-forget — uses `jobs` queue from #48)
Periodic (daily or threshold-based). The `usage-settlement-cron` Edge Function enqueues **one `jobs` row per `(vendor_org, batch_window)`** (type `settlement`) instead of doing transfers inline — at 1M vendors a single cron invocation cannot finish all transfers within an Edge Function timeout, and any partial completion without checkpointing = lost money. The job handler: aggregates unsettled `usage_events` for that scope → creates a Stripe transfer via `withStripeRetry` (#48 §8, handles 429/5xx) → marks events `settled_at` → audit. Idempotency key per batch ensures re-runs never double-transfer. Because credits were already collected at top-up, this only **distributes** already-held money. Reuse `transferVendorShare` / `transferResellerShare` / `transferAffiliateShare`.
